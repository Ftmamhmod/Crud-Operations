...
const data=[
  {id:crypto.randomUUID() ,amg:userImg, name:'Ahmed' , email:'karthi@gmmail.com' , phon:7305477760 ,enrollNumber:1234567305477760 , dete:'08-Dec, 2021'},
  {id:crypto.randomUUID() ,amg:userImg, name:'Ahmed' , email:'karthi@gmmail.com' , phon:7305477760 ,enrollNumber:1234567305477760 , dete:'08-Dec, 2021'},
  {id:crypto.randomUUID() ,amg:userImg, name:'Ahmed' , email:'karthi@gmmail.com' , phon:7305477760 ,enrollNumber:1234567305477760 , dete:'08-Dec, 2021'},
  {id:crypto.randomUUID() ,amg:userImg, name:'Ahmed' , email:'karthi@gmmail.com' , phon:7305477760 ,enrollNumber:1234567305477760 , dete:'08-Dec, 2021'},
  {id:crypto.randomUUID() ,amg:userImg, name:'Ahmed' , email:'karthi@gmmail.com' , phon:7305477760 ,enrollNumber:1234567305477760 , dete:'08-Dec, 2021'},
...
const Students = () => {
  ...
  <div className="flex flex-col sm:flex-row h-auto sm:h-15 w-11/12 m-auto  sm:items-center justify-between pt-2">
    ...
    <button className="bg-[#FEAF00] h-10 sm:h-full w-1/2  sm:w-[199px] rounded-xl text-white text-sm sm:text-base">
      ADD NEW STUDENT
    </button>
  </div>
  ...
  <div className="hidden sm:block mt-4 overflow-x-auto ">
    <table className="min-w-[700px] w-11/12 m-auto border-separate border-spacing-y-4">
      ...
      <tbody>
        {data.map((student) => (
          <tr className="h-18 text-center bg-white rounded-2xl shadow" key={student.id}>
            <td className="rounded-l-2xl">
              <img src={student.amg} alt="#" className="rounded-2xl size-12 ml-3" />
            </td>
            <td>{student.name}</td>
            <td>{student.email}</td>
            <td>{student.phon}</td>
            ...
            <td className="rounded-r-2xl">
              <div className="flex justify-center text-[20px] text-[#FEAF00]">
                <button className="mr-5"><GoPencil /></button>
    ...
    {data.map((student) => (
      <div key={student.id} className="bg-white rounded-2xl p-4 shadow ">
        <div className="flex items-center space-x-4">
          <img src={student.amg} alt="#" className="rounded-xl w-12 h-12" />
          <div>
        ...
        <div className="mt-3 text-sm text-gray-600">
          <p><span className="font-semibold">Phone:</span> {student.phon}</p>
          <p><span className="font-semibold">Enroll Number:</span> {student.enrollNumber}</p>
          <p><span className="font-semibold">Date of Admission:</span> {student.dete}</p>
        </div>
        <div className="flex justify-end mt-3 text-[#FEAF00] text-lg">
          <button className="mr-4"><GoPencil /></button>
          <button><MdOutlineDelete /></button>
        </div>
      </div>
    ))}
